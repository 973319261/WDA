...
<body class="layui-view-body">
	...
	<div id="flowStepModal" class="modal">
		<div class="layui-form layui-form-item" style="margin: 5px 0 10px;text-align: right;">
			<div class="btn-one">
				<button class="layui-btn layui-btn-sm" type="button" onclick="intsertFlowData(1)">添加</button>
				<button class="layui-btn layui-btn-sm layui-btn-warm" type="button" id="intsertFlowData(2)">修改</button>
				<button class="layui-btn layui-btn-sm layui-btn-danger" type="button" id="exportDidBtn">删除</button>
			</div>
			...
			<div class="layui-form-item">
				...
				<div class="layui-input-block">
			      <textarea placeholder="请输入内容" name="moduleFullName" class="layui-textarea" 
			      	required lay-verify="required" style="width:260px;min-height:150px;max-height:150px;resize: none;" 
			      	maxlength="100" ></textarea>
			      <span id="wordsLength" style="position:absolute; right:82px; bottom:0vh; font-size:12px; color:#BDCADA;">0/250</span>
			    </div>
			</div>
			<div class="layui-form-item" style="padding-top: 60px;text-align: center;">
				<button class="layui-btn layui-btn-sm" lay-submit lay-filter="saveResponseOrder">提交</button>
				<button type="reset" class="layui-btn layui-btn-sm layui-btn-primary" layui-btn-primary" id="resetAdd">重置</button>
			</div>
		</form>
	</div>
	...
	<script>
		...
		function searchResponseOrderInfo(){
			table.render({
				...
				cols : [ [ //标题栏
					...
					{ field : 'didConversionId',title : '响应指令',align : 'center',unresize : true,width:120,}, 
					{ field : 'didName',title : '详情',align : 'center',unresize : true,width:120},
				] ],done:function(res,curr,count){
					$('.layui-laypage-limits').hide();//隐藏分页选项下拉框
				}
			});
	 	...
	 	function editResponseOrder(num){
	 		...
	 		var responseOrderModal = layer.open({
				type : 1,
				title : txt,
		...
		form.on('submit(addSmallFlow)', function(data) {
			...
			if(field.stateOne==1){
				//新增--小流程具体数据新增
				$.post("${ctx}/dataManage/insertSmallFlowStepInfo.do",field,function(e){
					if(e.text=="success"){
