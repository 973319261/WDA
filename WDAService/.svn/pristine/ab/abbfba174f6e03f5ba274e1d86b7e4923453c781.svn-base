...
<html>
  <head>
    ...
    <title>My JSP 'editNotice.jsp' starting page</title>
		...
		<div class="layui-page-header">
			<div class="pagewrap">
				<span class="layui-breadcrumb"> 
					...
					<a href="javascript:void(0)"><cite>修改公告</cite></a> 
				</span>
						...
						<form class="layui-form" id="formInfo" action="">
							...
							<div class="layui-form-item">
							    <div class="layui-input-block" style="width: 100%;margin: 0px;">
							      <input class="layui-input" id="inputid" placeholder="请输入标题" name="noticeName" autocomplete="off" required 
									lay-verify="required" value="${noticeDetail.noticeName}" maxlength="150" style="padding-right: 80px;" />
							    </div>
							...
							<div class="layui-input-block" style="margin-left: 0px;">
								<textarea class="layui-textarea" placeholder="请输入内容(500字)" id="describeId" name="noticeDescribe"
									style="min-height: 140px;max-height: 140px;resize: none" required lay-verify="required" 
									maxlength="500" onkeyup="setLength(this,500,'wordsLengthEdit');">${noticeDetail.noticeDescribe}</textarea>
								<span id="wordsLengthEdit" style="position:absolute; right:5px; bottom:-3vh; font-size:12px; color:#BDCADA;">0/500</span>
							</div>
							...
							<input type="hidden" id="noticeIds" name="noticeId" value="${noticeDetail.noticeId}"  />
							<input type="hidden" id="fileInfosId" value="${noticeDetail.fileId}"  />
	...
	<script type="text/javascript">
		...
		var $ = layui.$;
 		...
 		$(function(){
 			...
 			var noticeName=$("#inputid").val();
 			var noticeDescribe=$("#describeId").val();
 			var noticeId=$("#noticeIds").val();
 			$.getJSON("${ctx}/share/cleanTemp.do",function(e){});
 			//附件信息回填
			$.getJSON("${ctx}/share/findFileInfoById.do",{noticeId:noticeId},function(e){});
 			...
 			table.render({
				...
				limit : 5,
				cols : [ [ //标题栏
					{ type:'numbers',field : 'number',title : '序号',align:'center',width:50},
					{ field : 'name',title : '附件名称',align:'center',unresize:true},
					{ field : 'fileTypeName',title : '附件类型',align:'center',minWidth : 100,unresize:true},
					{ field : 'tool',title : '操作',width : 100,align : 'center',toolbar : '#barDemo',unresize:true} 
				] ]
 			});
 			//设置公告标题的输入字数
 			var show = document.getElementById("numbers");
            show.innerHTML = Math.floor(noticeName.length);
            
          	//设置公告内容的输入字数
 			var showTwo = document.getElementById("wordsLengthEdit");
 			showTwo.innerHTML = 500-Math.floor(noticeDescribe.length)+"/500";
		});
		...
		form.on('submit(formDemo)',function(data){
			...
			if(fileInfo.length==0){
				...
				var fileId= $("#fileInfosId").val();
				var id=fileInfo[0].fileId;
				$.post("${ctx}/share/updateNoticeInfo.do",{noticeName:noticeName,noticeDescribe:noticeDescribe,fileTypeId:fileTypeId
					,fileName:fileName,noticeId:noticeId,fileId:fileId,id:id},function(e){
					if(e.text=="success"){
						layer.alert("修改成功！",{offset:'150px',icon:1,title:'提示'});
						setTimeout(function(){
		...
		//添加附件按钮
	 	...
	 	//取消按钮
		$("#ret").click(function(){
			layer.confirm("确定要取消当前页面的操作吗？",{icon:0,title:'提示',offset:'150px'},function(index){
				layer.close(index);
				$.getJSON("${ctx}/share/cleanTemp.do",function(e){});
				window.location.href="${ctx}/user/toUrl.do?page=/BasicData/shareIssue";
			});
