...
<title>My JSP 'vinTurnEskManage.jsp' starting page</title>
...
<style type="text/css">
	...
	#insertVinTurnEskModal .layui-input-inline{
		width: 240px;
	}
	
	#insertVinTurnEskModal label{
		width: 70px;
	}
</style>
</head>
...
<body class="layui-view-body">
		...
		<div class="layui-page-header">
			<div class="pagewrap">
				<span class="layui-breadcrumb"> <a href="javascript:void(0)">首页</a>
					...
					href="javascript:void(0)"><cite>Vin转Esk管理</cite> </a>
				</span>
				<h2 class="title">Vin转Esk管理</h2>
			</div>
		</div>
		<div class="layui-row">
							...
							<div class="layui-col-xs12 layui-col-sm12 layui-col-md12">
								...
								<div class="layui-col-xs2 layui-col-sm2 layui-col-md2" style="text-align: center;">
									<div class="btn-group">
										...
										<button class="layui-btn layui-btn-sm" id="addVinTurnEsk">添加数据</button>
									</div>
								</div>
							</div>
						</div>

						<table id="vinTurnEskInfo" lay-filter="vinTurnEskInfo"></table>
					</div>
				</div>
			</div>
		</div>
	...
	<!-- ******************************** 新增模态窗体  *********************** -->
	<div id="insertVinTurnEskModal" class="modal">
		<form class="layui-form" id="formVinTurnEsk" action="">
	...
	<script>
		...
		$(function() {
			...
			appendOptionName("arithmeticId", "${ctx}/security/findArithmeticInfo.do?algorithmType=3");
		...
		function searchArithmeticInfo() {
			...
			table.render({
				elem : '#vinTurnEskInfo',
				url : '${ctx}/security/findVinToEskInfo.do',
				where : {
		...
		//监听排序事件 
		table.on('sort(vinTurnEskInfo)', function(obj) { //注：sort 是工具条事件名		
			table.reload('vinTurnEskInfo', {
		...
		//监听工具条
		table.on('tool(vinTurnEskInfo)', function(obj) {
			var data = obj.data;
			if (obj.event == 'del') {
				layer.confirm('确定要删除该条信息？', {
	            	...
	            	layer.close(index); 
	            	$.post("${ctx}/security/deleteVinTurnEskInfo.do",{arithmeticId:data.arithmeticId},function(datas){
		...
		form.on('submit(formDemo)', function(data) {
			if(data.field.vehicleId == "0"){
				...
				$.getJSON("${ctx}/security/insertVinTurnEskInfo.do", data.field, function(d) {
					if (d.text=="success") {
						layer.closeAll();
