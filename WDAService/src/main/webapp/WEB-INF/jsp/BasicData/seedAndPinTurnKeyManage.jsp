...
<title>My JSP 'seedAndPinTurnKeyManage.jsp' starting page</title>
...
<body class="layui-view-body">
		...
		<div class="layui-page-header">
			<div class="pagewrap">
				<span class="layui-breadcrumb"> <a href="javascript:void(0)">首页</a>
					...
					href="javascript:void(0)"><cite>Seed&Pin转Key管理</cite> </a>
				</span>
				<h2 class="title">Seed&Pin转Key管理</h2>
			</div>
		</div>
		<div class="layui-row">
							...
							<div class="layui-col-xs12 layui-col-sm12 layui-col-md12">
								...
								<div class="layui-col-xs2 layui-col-sm2 layui-col-md2" style="text-align: center;">
									<div class="btn-group">
										...
										<button class="layui-btn layui-btn-sm" id="addSeedAndPinTurnKey">添加数据</button>
									</div>
								</div>
							</div>
						</div>
						<table id="seedAndPinTurnKeyInfo" lay-filter="seedAndPinTurnKeyInfo"></table>
					</div>
				</div>
			</div>
		</div>
	...
	<div id="insertModal" class="modal">
		<form class="layui-form" id="formSeedAndPinTurnKey" action="">	
			...
			<div class="layui-form-item">
				<label class="layui-form-label">Detal</label>
				<div class="layui-input-inline input-box arithmeticDelta">
					<select id="details" name="arithmeticDelta">
	...
	<script>
		...
		$(function() {
			...
			appendOptionName("arithmeticId", "${ctx}/security/findArithmeticInfo.do?algorithmType=4");
		...
		function searchArithmeticInfo() {
			...
			table.render({
				elem : '#seedAndPinTurnKeyInfo',
				url : '${ctx}/security/findSeedAndPinToKeyInfo.do',
				where : {
		...
		//监听排序事件 
		table.on('sort(seedAndPinTurnKeyInfo)', function(obj) { //注：sort 是工具条事件名	
			table.reload('seedAndPinTurnKeyInfo', {
		...
		//监听工具条
		table.on('tool(seedAndPinTurnKeyInfo)', function(obj) {
			var data = obj.data;
			if (obj.event == 'del') {
				layer.confirm('确定要删除该条信息？', {
	            	...
	            	layer.close(index); 
	            	$.post("${ctx}/security/deleteSeedAndPinTurnKeyInfo.do",{arithmeticId:data.arithmeticId},function(datas){
		...
		/*************************** 新增数据  *********************************************/
		$("#addSeedAndPinTurnKey").click(function() {
			showlayer();
		});		
		...
		form.on('submit(formDemo)', function(data) {
			if(data.field.vehicleId == "0"){
				...
				$.getJSON("${ctx}/security/insertSeedAndPinToKeyInfo.do", data.field, function(d) {
					if (d.text=="success") {
						layer.closeAll();
