...
public class DtcFragment extends Fragment {
    ...
    public View onCreateView(@NonNull LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        ...
        //获取控件
        tlTitle = view.findViewById(R.id.tl_dtc_title);
        ViewPager2 vpOrderContainer = view.findViewById(R.id.vp_dtc_container);
     ...
     */
        ...
        if (jurisdictionList.get(2).getIsEnable()) {//查询故障码
            title.add(getResources().getString(R.string.text_dtc_query));
        }
        if (jurisdictionList.get(3).getIsEnable()) {//读/清故障码
            title.add(getResources().getString(R.string.text_dtc_read));
        }
    ...
    class MyFragmentStateAdapter extends FragmentStateAdapter {
        ...
        public Fragment createFragment(int position) {
            if (fragments[position] == null) {
                ...
                //创建Fragment
                DtcStateFragment dtcStateFragment = new DtcStateFragment();
                //设置参数
                dtcStateFragment.setArguments(bundle);
                fragments[position] = dtcStateFragment;
