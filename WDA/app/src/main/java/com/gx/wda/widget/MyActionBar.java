...
public final class MyActionBar extends LinearLayout {
    ...
    private BadgeHelper badgeLeft;
    private BadgeHelper badgeRight;
    ...
    private void init(Context context) {
        ...
        btnRight=contentView.findViewById(R.id.btn_actionbar_right);
    ...
    public void setData(Activity context,String strTitle, int resIdLeft, int resIdRight,int intColor,int backgroundColor, final ActionBarClickListener listener) {
        ...
        if (resIdLeft == 0) {
            ...
            ivLeft.setBackgroundResource(resIdLeft);
            ivLeft.setVisibility(View.VISIBLE);
            badgeLeft = new BadgeHelper(context);
            badgeLeft.setBadgeType(BadgeHelper.Type.TYPE_POINT)
                    .setBadgeOverlap(false)
                    .bindToTargetView(tvLeft);
            badgeLeft.setBadgeEnable(false);
        }
        if (resIdRight == 0) {
            ivRight.setVisibility(View.GONE);
        } else {
            ivRight.setBackgroundResource(resIdRight);
            ivRight.setVisibility(View.VISIBLE);
            badgeRight = new BadgeHelper(context);
            badgeRight.setBadgeType(BadgeHelper.Type.TYPE_POINT)
                    .setBadgeOverlap(false)
                    .bindToTargetView(tvRight);
            badgeRight.setBadgeEnable(false);
        }
     ...
     * 设置消息红点
     */
    public void setBadgeLeftShow(){
        badgeLeft.setBadgeEnable(true);
    }
    public void setBadgeLeftHide(){
        badgeLeft.setBadgeEnable(false);
    }
    public void setBadgeRightShow(){
        badgeRight.setBadgeEnable(true);
    }
    public void setBadgeRightHide(){
        badgeRight.setBadgeEnable(false);
    }
    ...
    public void checkAll(ExportMessageAdapter shareFileAdapter){
        tvLeft.setOnClickListener(new OnClickListener() {
            @Override
            public void onClick(View v) {
                if (isCheck){
                ...
                shareFileAdapter.isCheckAll(!isCheck);
                isCheck=!isCheck;
            }
        });
    }
